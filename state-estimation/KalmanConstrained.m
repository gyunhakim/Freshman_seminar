...
N = round(tf / T); % number of time steps
...
xarray = zeros(4, N+1);
xarray(:, 1) = x;
xhatarray = zeros(4, N+1);
xhatarray(:, 1) = xhat;
Constrarray = zeros(2, N+1);
Constrarray(:, 1) = D * xhat;
xhat1array = zeros(4, N+1);
xhat1array(:, 1) = xhat1;
Constr1array = zeros(2, N+1);
Constr1array(:, 1) = D * xhat1;
ConstrTildearray = zeros(2, N+1);
ConstrTildearray(:, 1) = D * xtilde;
ConstrTildeParray = zeros(2, N+1);
ConstrTildeParray(:, 1) = D * xtildeP;
xtildearray = zeros(4, N+1);
xtildearray(:, 1) = xtilde;
xtildeParray = zeros(4, N+1);
xtildeParray(:, 1) = xtildeP;
...
for k = 1 : N
    ...
    K = P * H' / (H * P * H' + R); % Kalman gain for standard filter
    K1 = P1 * H1' / (H1 * P1 * H1' + R1); % Kalman gain for perfect measurement filter
    ...
    xtilde = xhat - D' / (D*D') * D * xhat;
    xtildeP = xhat - P * D' / (D*P*D') * D * xhat;
    % Save data in arrays.
    xhatarray(:, k+1) = xhat;
    xhat1array(:, k+1) = xhat1;
    xtildearray(:, k+1) = xtilde;
    xtildeParray(:, k+1) = xtildeP;
    Constrarray(:, k+1) = D * xhat;
    Constr1array(:, k+1) = D * xhat1;
    ConstrTildearray(:, k+1) = D * xtilde;
    ConstrTildeParray(:, k+1) = D * xtildeP;
    xarray(:, k+1) = x;
end
...
figure;
plot(t, abs(xarray(2, :) - xhatarray(2, :)), 'r:', t, abs(xarray(2, :) - xtildearray(2, :)), 'b-');
set(gca,'FontSize',12); set(gcf,'Color','White');
xlabel('seconds'); ylabel('east position estimation error');
legend('unconstrained', 'constrained');
