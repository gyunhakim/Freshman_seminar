...
pos_ini = [5.3 3.6];
% initial state
x = [pos_ini(1) pos_ini(2) 0]'; % 빈공간에 속도를 추가가능, 속도값으로 필터의 성능을 알 수 있음.-> 속도 성능이 지정값에 가까우면 필터성능이 좋다고 볼 수 있음
...
% matrix A
A = [0.9305 0 0; 0.0077 0.9802 -0.0173; 0.0142 0 0.8953]; % 다룰 값에 따라 열의 값 수정
% matrix C
C = [1 0 0; 0 1 0]; % 다룰 값에 따라 열의 값 수정
...
for k = 1:size(saved_pos,2)
    ...
    %Q=0;
    %R=0;  
    Q = 0.0001*eye(3); %A*P*A에 4x4여서 사이즈를 맞춰야 한다. 4x1이면 자동으로 열이 복사되어 4x4가 된다. 웬만하면 단위행렬 형태로.
    R = 0.01*eye(2); %C*Pk_predict*C'가 2x2행렬이라서 사이즈를 맞춰야 한다. 2x1이면 자동으로 열하나가 복사되어 2x2가 된다. 이게 작아질수록 update 성능이 올라간다.
    % Q,R 비율중요.
    % measurement
    y = saved_pos(:,k);
...
plot(x_predict_trajectory(1,:), x_predict_trajectory(2,:), '-ob', 'MarkerSize', 5); hold on;
plot(x_update_trajectory(1,:), x_update_trajectory(2,:), '-+b', 'MarkerSize', 5); hold on;
grid on;
%xlim([-1 7]); ylim([3 7]);
xlabel('x(m)'); ylabel('y(m)');
legend('measurement', 'noisy measurement','predict','update');
