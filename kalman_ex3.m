...
for k = 1:size(saved_pos,2)
    ...
    %Q=0;
    %R=0;  
    Q = 0.1*eye(3); %A*P*A에 4x4여서 사이즈를 맞춰야 한다. 4x1이면 자동으로 열이 복사되어 4x4가 된다. 웬만하면 단위행렬 형태로.
...
%   y = saved_pos_noise(:,k); % measurement값에 noise를 섞어 현실성을 부여
    delta = 0.01*randn(1); % modeling uncertainites
    saved_delta(:,k) = delta;
    % matrix A
    A = [0.9305+delta 0 0; 0.0077 0.9802+delta -0.0173; 0.0142 0 0.8953+delta];
    % matrix C
    C = [1+delta 0 0; 0 1+delta 0];
    %% Kalman filter
